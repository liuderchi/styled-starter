...
const ChooseThemeModal = (props) => (
  <Modal
    ...
    onRequestClose={() => props.update(enableChooseThemeModal(false))}
    isOpen={true}
    contentLabel="Edit Theme Modal"
    style={styles}
  ...
  >
    <Div mw="320px" p={4}>
      <Div
        id="closeThemeModal"
        onClick={() => props.update(enableChooseThemeModal(false))}
        f={2}
        py={2}
        px={3}
        position="absolute"
      ...
      <Div>
        <Select
          id="selectTheme"
          onChange={(e) => props.update(updateTheme(e.target.value))}
          style={{ display: 'inline-block', padding: '8px' }}
          w={160}
        >
          {Object.keys(Object.assign({ 'Select...': '' }, props.themes)).map(
            (theme) => (
              <option key={theme} value={props.themes[theme] && theme}>
                {theme}
              </option>
            )
          )}
